...
{{SERIES_TOTAL}} series from ten Monash collections and {{CONFIRMATION_WINDOWS}} sealed confirmation
...
{{PRIMARY_PERCENT}}% relative to PatchTST (95% hierarchical moving-block interval
{{PRIMARY_CI}}); it revised {{CONFIRMATION_COVERAGE}}% of forecasts and improved
{{DATASET_WINS}}/10 datasets. Under the registered 1% smallest effect size of interest, the decision
was **{{DECISION}}**. The exact self-inclusion stress test, provenance ablations, 4,000-draw
...
This yields {{SERIES_TOTAL}} series. The committed `series_selection.csv` preserves identifiers,
...
by greater coverage, smaller (k), then larger temperature. The selected values were (k={{K}}\),
(T={{TEMPERATURE}}\), threshold {{THRESHOLD}}, with development MSE change
{{DEVELOPMENT_PERCENT}}% and coverage {{DEVELOPMENT_COVERAGE}}%.
...
{{DEVELOPMENT_PERCENT}}% development MSE change relative to PatchTST. Figure 13 displays the entire
...
The locked candidate revised {{CONFIRMATION_COVERAGE}}% of confirmation windows. Its paired MSE
reduction was {{PRIMARY_PERCENT}}%, with a 95% hierarchical interval {{PRIMARY_CI}} and bootstrap
probability of positive effect {{PROBABILITY_POSITIVE}}. It improved {{DATASET_WINS}} of ten dataset
...
**{{DECISION}}**, based on an MSE effect of {{PRIMARY_PERCENT}}% ({{PRIMARY_CI}}), the 1% SESOI,
{{DATASET_WINS}} dataset wins, and robustness metrics. This classification should be preserved even
...
collections, the selected method produced {{PRIMARY_PERCENT}}% confirmation MSE change with interval
{{PRIMARY_CI}}, leading to the preregistered decision **{{DECISION}}**.
